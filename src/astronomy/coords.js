...
export function getPrecessionAngles(jde) {
  ...
  // IAU 2006 岁差角（角秒 -> 弧度）
  const zetaA = (2306.083227 * T + 0.29885 * T2 + 0.018042 * T3) * ARCSEC2RAD;
...
export function getNutation(jde) {
  ...
  // 平黄赤交角
  const eps0 =
    (84381.448 - 46.815 * T - 0.00059 * T2 + 0.001813 * T2 * T) * ARCSEC2RAD;
...
export function applyNutation(ra, dec, jde) {
  ...
  // 章动对赤经的影响
  const dRa =
    (Math.cos(eps) + Math.sin(eps) * sinRa * Math.tan(dec)) * dPsi -
...
export function applyAnnualAberration(ra, dec, jde) {
  ...
  // 太阳平黄经
  const L = (280.46 + 36000.77 * T) * DEG2RAD;
  // 地球轨道偏心率
  const e = 0.016708634 - 0.000042037 * T;
  // 太阳平近点角
  const g = (357.529 + 35999.05 * T) * DEG2RAD;
  ...
  const dRa =
    (k * (cosL * cosRa * sinDec - sinL * sinRa * sinDec)) / cosDec +
    (e * k * (cosG * cosRa * sinDec - sinG * sinRa * sinDec)) / cosDec;

  const dDec =
    k *
      (cosL * (sinRa * sinDec * cosDec - cosRa * sinDec * sinDec) +
        sinL * cosDec * cosDec) +
    e *
      k *
      (cosG * (sinRa * sinDec * cosDec - cosRa * sinDec * sinDec) +
        sinG * cosDec * cosDec);
 ...
 * 输出: 视高度角修正量（度）
 */
export function getAtmosphericRefraction(
  altitude,
  pressure = 1010,
  temperature = 10,
) {
  ...
  // Saemundsson 公式
  const R =
    1.02 / Math.tan((altitude + 10.3 / (altitude + 5.11)) * DEG2RAD) / 60;
...
export function galacticToEquatorial(l, b) {
  ...
  const ra =
    Math.atan2(cosB * cosL, sinB * cosDecGP - cosB * sinL * sinDecGP) + raGP;
