...
const PLANET_ELEMENTS = {
  ...
  venus: {
    ...
    omega: [76.67992, 0.90112, 0.0004066, -0.00000008],
  ...
  earth: {
    ...
    i: [0.0, 0.0, 0.0, 0.0],
    omega: [174.873174, -0.24109, 0.00004262, 0.000000001],
  ...
  mars: {
    ...
    i: [1.849726, -0.000601, 0.00001276, -0.000000006],
  ...
  jupiter: {
    ...
    i: [1.30327, -0.0054966, 0.00000465, -0.000000004],
...
function getPlanetElements(planet, T) {
  ...
  const e =
    typeof elem.e === "number"
      ? elem.e
      : elem.e[0] + elem.e[1] * T + elem.e[2] * T2 + elem.e[3] * T3;
  const i = elem.i[0] + elem.i[1] * T + elem.i[2] * T2 + elem.i[3] * T3;
  const omega =
    elem.omega[0] + elem.omega[1] * T + elem.omega[2] * T2 + elem.omega[3] * T3;
...
export function getPlanetHeliocentric(planet, jde) {
  ...
  // 日心黄纬（考虑轨道倾角）
  const lat = elem.i * Math.sin(nu); // 简化：黄纬 = 倾角 * sin(真近点角)
  // 更精确的计算需要考虑升交点经度，但简化模型中假设升交点与近日点重合
  // 实际应用中，对于低精度需求，lat ≈ 0 是可接受的
  // 这里使用更精确的近似：考虑轨道倾角和升交点
  const trueLat =
    Math.asin(Math.sin(elem.i) * Math.sin(elem.pi + nu - elem.omega)) || 0;
...
export function getPlanetEquatorial(planet, jde) {
  ...
  const sunDist = Math.sqrt(
    (earth.r * Math.cos(earth.lon)) ** 2 + (earth.r * Math.sin(earth.lon)) ** 2,
  );
  const elongation =
    Math.acos(
...
export function getSunEquatorial(jde) {
  ...
  // 太阳距离（AU）
  const R =
    (1.000001018 * (1 - 0.016708634 * 0.016708634)) /
