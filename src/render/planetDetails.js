...
// 木星卫星数据
const JUPITER_MOONS = [
  { name: '木卫一 (Io)', distance: 0.003, period: 1.769, size: 0.0025, color: 0xffff99 },
  { name: '木卫二 (Europa)', distance: 0.005, period: 3.551, size: 0.0022, color: 0xaaccff },
  { name: '木卫三 (Ganymede)', distance: 0.008, period: 7.155, size: 0.0035, color: 0x887766 },
  { name: '木卫四 (Callisto)', distance: 0.014, period: 16.689, size: 0.0032, color: 0x554433 }
];
...
export class PlanetDetailsRenderer {
   ...
   * 创建木星卫星系统
   */
  createJupiterMoons(jupiterPosition) {
   ...
   * 创建土星环系统
   */
  createSaturnRings(saturnPosition) {
    const group = new THREE.Group();
  ...
  createMoonFeatures() {
    ...
    for (const mare of maria) {
      const gradient = ctx.createRadialGradient(mare.x, mare.y, 0, mare.x, mare.y, mare.r);
  ...
  updateMoonPhase(moonMesh, phase) {
    ...
    if (phase < 0.5) {
      // 新月到满月
      ctx.ellipse(256, 128, Math.abs(terminatorX - 256), 128, 0, 0, Math.PI * 2);
      ctx.fill();
    } else {
      // 满月到新月
